...
if canopy.get('sourcePeriod')!={'historicImageryYear':2012,'currentImageryYear':2022}:errors.append('canopy-assessment: expected 2012-2022 comparison period')
for key in ('strata','tractCanopy','priorityTracts'):
 if not canopy.get(key):errors.append(f'canopy-assessment: {key} must not be empty')
for key in ('trees','canopyAcres','annualBenefitsUsd','canopyCoveragePct'):
 if not isinstance(canopy.get('summary',{}).get(key),(int,float)) or canopy['summary'][key]<=0:errors.append(f'canopy-assessment: invalid summary {key}')
canopy_text=json.dumps(canopy).lower()
for forbidden_key in ('ownername','mailaddress','fulladdress'):
 if forbidden_key in canopy_text:errors.append(f'canopy-assessment: privacy field {forbidden_key} present')
...
for proposal in review_proposals.get('proposals',[]):
 ...
 for key in ('id','proposedBy','proposedAt','field','proposedValue','reason','sourceUrl','status'):
  if not proposal.get(key):errors.append(f'review proposal {proposal.get("id")}: missing {key}')
 if proposal.get('status') not in ('pending','accepted','declined','needs_information'):errors.append(f'review proposal {proposal.get("id")}: invalid status')
